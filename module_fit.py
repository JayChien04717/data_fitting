...
def spectrum_analyze(x: float, y: float, fit: bool = True) -> float:
    ...
    if fit == True:
        ...
        plt.axvspan(res-pOpt[3], res+pOpt[3], alpha=0.5,
                    label=f'lw = {2*pOpt[3]/1e6}MHz')
        plt.legend()
        return res
...
def dispersive_analyze(x: float, y1: float, y2: float, fit: bool = True):
    ...
    if fit == True:
        plt.plot(x, asym_lorfunc(x, *pOpt1),
                 label=f'fite, $\kappa$ = {pOpt1[3]/1e6:.2f}MHz')
        plt.plot(x, asym_lorfunc(x, *pOpt2),
                 label=f'fitg, $\kappa$ = {pOpt2[3]/1e6:.2f}MHz')
...
def T2r_analyze(x: float, y: float, fit: bool = True, normalize: bool = False):
    ...
    if fit == True:
        plt.plot(x, sim, label=f'fit, R square = {r_square:.2f}')
    plt.title(
        f'T2r = {pOpt[3]*1e6:.2f}$\mu s, detune = {pOpt[1]/1e6:.2f}MHz \pm {(error[1])/1e3:.2f}kHz$', fontsize=15)
    plt.xlabel('$t\ (\mu s)$', fontsize=15)
    if normalize == True:
        plt.ylabel('Population', fontsize=15)
    plt.legend()
